...
import { TIC_GAS_WEB_APP_URL } from './googleServices';

const TIC_SHEET_ID = "16zyQneOwbnWXj8h42CP-ABv75UCkYQOLIATiugFu9Aw";
const TIC_SHEET_NAME = "Incidències TIC";
...
export const getIncidents = async (accessToken) => {
  if (isMobile()) {
    ...
    const range = `${TIC_SHEET_NAME}!A:I`;
    const data = await fetchSheetData(range, accessToken, TIC_SHEET_ID);
    ...
    return new Promise((resolve, reject) => {
      ...
      script.src = `${TIC_GAS_WEB_APP_URL}?action=getIncidents&callback=${callbackName}&token=${accessToken}`;
...
export const addIncident = async (incidentData, accessToken) => {
  if (isMobile()) {
    // Mobile: Use Google Sheets API with specific ID
    const range = `${TIC_SHEET_NAME}!A:I`;
    const headers = await fetchSheetData(`${TIC_SHEET_NAME}!A1:I1`, accessToken, TIC_SHEET_ID);
    ...
    return await appendSheetData(range, [newRow], accessToken, TIC_SHEET_ID);
  } else {
    // Desktop: Use GAS Web App
    return new Promise((resolve, reject) => {
      const callbackName = 'jsonp_callback_' + Math.round(100000 * Math.random());
      ...
      const script = document.createElement('script');
      script.src = `${TIC_GAS_WEB_APP_URL}?action=addIncident&data=${encodeURIComponent(JSON.stringify(incidentData))}&callback=${callbackName}&token=${accessToken}`;
      script.onerror = () => reject(new Error('Error loading GAS script.'));
      document.body.appendChild(script);
    });
  }
...
export const updateIncident = async (incidentData, accessToken) => {
    if (isMobile()) {
        ...
        const range = `${TIC_SHEET_NAME}!A${rowIndex}:I${rowIndex}`;
        const headers = await fetchSheetData(`${TIC_SHEET_NAME}!A1:I1`, accessToken, TIC_SHEET_ID);
        const updatedRow = headers[0].map(header => {
            if (header === "Data de la darrera edició") return new Date().toISOString();
            return incidentData[header] || "";
        });
        return await updateSheetData(range, [updatedRow], accessToken, TIC_SHEET_ID);
    } else {
        // Desktop: Use GAS Web App
        return new Promise((resolve, reject) => {
            const callbackName = 'jsonp_callback_' + Math.round(100000 * Math.random());
            ...
            const script = document.createElement('script');
            script.src = `${TIC_GAS_WEB_APP_URL}?action=updateIncident&data=${encodeURIComponent(JSON.stringify(incidentData))}&callback=${callbackName}&token=${accessToken}`;
            script.onerror = () => reject(new Error('Error loading GAS script.'));
            document.body.appendChild(script);
        });
    }

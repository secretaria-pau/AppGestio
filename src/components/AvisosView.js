...
import { getAllAvisos, toggleAvisoStatus, deleteAviso, addAviso } from '../avisosService';
...
const AvisosView = ({ onBackClick, profile, accessToken }) => {
  ...
  const fetchAllAvisos = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const allAvisos = await getAllAvisos(accessToken); // Get all avisos
      setAvisos(allAvisos);
    } catch (err) {
  ...
  const handleToggleStatus = async (id) => {
    try {
      await toggleAvisoStatus(accessToken, id);
  ...
  const handleConfirmDelete = useCallback(async () => {
    if (avisoToDelete) {
      try {
        await deleteAviso(accessToken, avisoToDelete);
